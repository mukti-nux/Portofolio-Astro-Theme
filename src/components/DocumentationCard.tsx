...
export default function DocumentationCard({ title, description, imageUrl, link }: DocProps) {
  return (
    <div class="relative max-w-2xl mx-auto mt-16">
      ...
      <div class="min-h-[230px] relative rounded-xl p-6 bg-white dark:bg-gray-900 shadow-lg transition-all duration-300 border border-transparent hover:border-[3px] hover:border-[rgb(0,255,255)] group flex flex-col sm:flex-row items-center sm:items-start space-y-4 sm:space-y-0 sm:space-x-6 overflow-hidden">
        <img
          src={imageUrl}
          alt={`thumbnail ${title}`}
